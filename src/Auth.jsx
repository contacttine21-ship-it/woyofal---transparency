    ...
    <div className="wrap">
      <div className="brand">
        ...
        <div>
          ...
          <div className="brand-tag">{mode === 'signup' ? 'Créer un compte' : 'Se connecter'}</div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="auth-tabs">
          <button
            ...
            onClick={() => { setMode('signup'); setErreur(''); }}
          >
            Inscription
          </button>
          <button
        ...
        {mode === 'signup' ? (
          <form onSubmit={handleSignup}>
            <div className="row2">
              <div className="field">
                <label>Prénom</label>
                <input type="text" value={prenom} onChange={e => setPrenom(e.target.value)} placeholder="Awa" />
              </div>
              <div className="field">
                <label>Nom</label>
                <input type="text" value={nom} onChange={e => setNom(e.target.value)} placeholder="Diop" />
              </div>
            </div>
            <div className="field">
              <label>Numéro de téléphone</label>
              <input type="tel" value={telephone} onChange={e => setTelephone(e.target.value)} placeholder="77 123 45 67" />
            </div>
            <div className="field">
              <label>E-mail</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="awa.diop@exemple.com" />
            </div>
            <div className="field">
              <label>Mot de passe</label>
              <input type="password" value={motDePasse} onChange={e => setMotDePasse(e.target.value)} placeholder="6 caractères minimum" />
            </div>
            {erreur && <div className="auth-error">{erreur}</div>}
            <button className="btn teal" type="submit" disabled={chargement}>
          ...
          <form onSubmit={handleLogin}>
            ...
            <div className="field">
              <label>Mot de passe</label>
              <input type="password" value={motDePasse} onChange={e => setMotDePasse(e.target.value)} placeholder="Ton mot de passe" />
            </div>
            {erreur && <div className="auth-error">{erreur}</div>}
            <button className="btn teal" type="submit" disabled={chargement}>
