...
export default function App({ profil, uid, donneesInitiales }) {
  ...
  const [syncEtat, setSyncEtat] = useState('idle'); // idle | saving | saved
  ...
  // Sauvegarde automatique vers Firestore (avec un léger délai pour grouper les frappes rapides)
  useEffect(() => {
    if (!uid) return;
    setSyncEtat('saving');
    ...
    <div className="wrap">
      ...
      {syncEtat !== 'idle' && (
        <div className="sync-note">
          {syncEtat === 'saving' ? 'Synchronisation…' : 'Synchronisé ✓'}
        </div>
      )}
