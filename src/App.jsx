...
function ConsoChart({ points }) {
  ...
  useEffect(() => {
    ...
    // Ligne colorée segment par segment selon la tranche traversée
    for (let i = 0; i < points.length - 1; i++) {
      const p1 = points[i], p2 = points[i + 1];
      const mid = (p1.cumul + p2.cumul) / 2;
      const key = mid <= 150 ? 't1' : mid <= 250 ? 't2' : 't3';
    ...
    points.forEach((p, i) => {
      const key = p.cumul <= 150 ? 't1' : p.cumul <= 250 ? 't2' : 't3';
      ctx.fillStyle = '#fff';
      ctx.strokeStyle = TRANCHE_COLOR[key];
      ctx.lineWidth = 2;
      ctx.beginPath();
...
function BottomNav({ tab, setTab }) {
  const items = [
    ...
    { id: 'recharge', icon: 'ti-bolt', label: 'Recharge' },
    { id: 'suivi', icon: 'ti-chart-line', label: 'Suivi' },
    ...
    <div className="app-shell">
      <div className="topbar">
        <div>
          ...
          <div className="name">Woyofal Transparency</div>
        </div>
        <div className="bolt"><i className="ti ti-bolt" aria-hidden="true"></i></div>
      </div>
      {syncEtat !== 'idle' && (
      ...
      {tab === 'accueil' && (
        <div className="wrap" style={{ paddingTop: 10 }}>
          <div className="hero">
            ...
            <div className="hero-sub">pour {fmtF(montant)} payés</div>
            <div className="hero-row">
              <div className="hero-tag">
                <i className="ti ti-bolt" aria-hidden="true"></i>
                {res.dernierTranche ? res.dernierTranche.nom : '—'}
              </div>
          ...
          <div className="quick-list">
            <div className="quick-row" onClick={() => setTab('suivi')}>
              <div className="left">
                <div className="icon"><i className="ti ti-calendar" aria-hidden="true"></i></div>
                <div>
                  <div className="label">Cumul ce mois</div>
                  <div className="sub">{consoPoints.length} entrée{consoPoints.length > 1 ? 's' : ''} de suivi</div>
                </div>
              </div>
        ...
        <div className="wrap" style={{ paddingTop: 10 }}>
          <div className="card">
            ...
            <div className="pill-toggle">
              ...
              <button className={modeSaisie === 'scan' ? 'active' : ''} onClick={() => setModeSaisie('scan')} disabled>Scanner ticket</button>
            </div>
        ...
        <div className="wrap" style={{ paddingTop: 10 }}>
          ...
          <div className="menu-list">
            ...
            <div className="menu-row">
              ...
              <div className="txt"><div className="t">À propos</div><div className="s">Woyofal Transparency · v0.1</div></div>
            </div>
          </div>
